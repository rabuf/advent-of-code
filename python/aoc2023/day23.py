...
from networkx import Graph, DiGraph, all_simple_paths, path_weight
...
def create_graph(lines, climbable=False):
    ...
    for pos in nodes:
        ...
        if climbable or nodes[pos] == '.':
            neighbors = [pos + delta for delta in [1, -1, len(lines[0]), -len(lines[0])]]
        if not climbable:
            match nodes[pos]:
                case '<':
                    neighbors = [pos - 1]
    ...
    while removed:
        ...
        for n in curr.nodes:
            if curr.degree(n) == 2:
                ...
                l, r = neighbors
                lw = curr.get_edge_data(l, n)
                rw = curr.get_edge_data(r, n)
                curr.add_edge(neighbors[0], neighbors[1], weight=lw['weight'] + rw['weight'])
                curr.remove_node(n)
                break
    return curr
...
def main():
    ...
    with open(input_dir / "2023" / "23.txt") as f:
        ...
        p1 = max(path_weight(graph, path, 'weight') for path in all_simple_paths(graph, start, end))
        p2 = max(path_weight(cg, path, 'weight') for path in all_simple_paths(cg, start, end))
        print_day(23, p1, p2)
