...
def smudge(field: list[str], vertical, horizontal):
    for i in range(len(field)):
        for j in range(len(field[0])):
            new_field = field[:]
            new_field[i] = (
                field[i][:j] + ("." if field[i][j] == "#" else ".") + field[i][j + 1 :]
            )
...
def main(input_dir=Path(sys.argv[1])):
    with open(input_dir / "2023" / "13.txt") as f:
        ...
        smudged_reflections = [
            smudge(field, *reflection)
            for field, reflection in zip(fields, reflection_points)
        ]
        ...
        print_day(
            13, vertical + 100 * horizontal, smudged_vertical + 100 * smudged_horizontal
        )
