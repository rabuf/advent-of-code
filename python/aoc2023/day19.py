...
def parse_condition_range(condition):
    ...
    def less_than(part):
        ...
        if lower < value < upper:
            return {**part, category: (lower, value - 1)}, {
                **part,
    ...
    def greater_than(part):
        lower, upper = part[category]
        if lower < value < upper:
            return {**part, category: (value + 1, upper)}, {
                **part,
...
def apply_workflows_range(part, workflows):
    ...
    while search_space:
        ...
        if "A" in next_spaces:
            result += sum(
                reduce(mul, (upper - lower + 1 for lower, upper in part.values()))
                for part in next_spaces["A"]
            )
        search_space.extend(
            (to, part)
            for to, parts in next_spaces.items()
            if to != "A"
            for part in parts
        )
    return result
...
def main(input_dir=Path(sys.argv[1])):
    ...
    p2_accepted = apply_workflows_range(
