...
moved at the same time. At least one object must be in the eleverator
...
top. That's way low becaues it doesn't count the necessary backwards
...
the input so I don't accidentalyl change it. IT worked fine on first
run, broke on second.
...
lists just aren't effecient.

I'm giving up on the above code. It's just too slow for the second
part. I'ts been running for almoust 3 hours now and still hasn't
